...
county_select<-county_shape[county_shape$ADMIN1 == "Baringo",]
...
### Reporting Summaries for `r params$county_name` County for `r paste0("20", params$time)`
...
```{r, eval = TRUE, fig.show= "hold", out.width = "50%", fig.align = 'center', fig.cap = "Deciduous and evergreen forest cover"}
...
# use tmap instead
tm_shape(decid_prj, bbox = county_select) + 
  tm_raster(palette = "Greens", style = "cont", title = "% Change") + 
  tm_shape(county_shape) +
  tm_borders()+
  tm_layout(legend.outside = TRUE, frame = FALSE, title = "Decidious Forest", title.position = c("left", "top"))

tm_shape(ever_prj, bbox = county_select)+ 
  tm_raster(palette = "Greens", style = "cont", title = "% Change") + 
  tm_shape(county_shape) +
  tm_borders()+
  tm_layout(legend.outside = TRUE, frame = FALSE, title = "Evergreen Forest", title.position = c("left", "top"))
...
test <- params$clim_inputs
reps <- test$areaCounty
